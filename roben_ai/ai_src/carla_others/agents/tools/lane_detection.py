...
class LaneDetection:
    ...
    def perspective_transform(self, img):
        ...
        src = np.array(
            ...
            [688, y_size * 0.55],
            [1025, y_size * 1],
            [165, y_size * 1]], dtype=np.float32)
        # Destination points
        dst = np.array(
            [[offset, 0],
            [x_size - offset, 0],
            [x_size - offset, y_size],
            [offset, y_size]], dtype=np.float32)
    ...
    def sliding_windows(image):
        ...
        midpoint = np.int64(image.shape[1] // 2)
        ...
        # Candidate pixels
        window_height = np.int64(image.shape[0] // no_windows)
        ...
        for window in range(no_windows):
            ...
            if len(good_right_inds) > min_pix:
                right_current = np.int64(np.mean(nonzero_x[good_right_inds]))
            if len(good_left_inds) > min_pix:
                left_current = np.int64(np.mean(nonzero_x[good_left_inds]))
    ...
    def fit_and_cal_curvature_offset(self, image):
        ...
        ploty = np.linspace(0, image.shape[0] - 1, image.shape[0], dtype=np.int64_)
        left_fitx = np.int64_(np.clip(left_fit[0] * ploty ** 2 + left_fit[1] * ploty + left_fit[2], 0, image.shape[1] - 1))
        right_fitx = np.int64_(
