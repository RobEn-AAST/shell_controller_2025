...
class BirdeyeRenderer:
    ...
    def _render_ego_vehicle(self, **env_state):
        ...
        try:
            ...
            print(f'failed to render {self._ego.id}')
    ...
    def _render_background_vehicles(self, **env_state):  
        ...
        # Provide a default color if 'background_vehicles_color' is not in env_state  
        # or if it's None. Color.BLUE is an example, you can choose any Color constant.  
        colors_map = env_state.get("background_vehicles_color", {})   
        if not colors_map: # If the map is empty or None, use a default for all  
            default_vehicle_color = Color.BLUE # Hardcode blue for all background vehicles  
            colors_map = {vehicle_id: default_vehicle_color for vehicle_id, _ in self._world_manager.actor_polygons.items()}  
        ...
        for vehicle_id, polygon in vehicle_polygons.items():  
            ...
            # Get the specific color for the vehicle, or use the default if not found  
            vehicle_color = colors_map.get(vehicle_id, default_vehicle_color)   
