...
class CarlaEnv(gym.Env):
    ...
    def __init__(self, params):
        ...
        if self.discrete:
            self.action_space = spaces.MultiDiscrete([self.n_acc, self.n_steer])
        else:
    ...
    def step(self, action):
        ...
        if self.discrete:
            acc = self.discrete_act[0][int(action[0])]
        ...
        return (self._get_obs(), self._get_reward(), terminated, truncated, copy.deepcopy(info))
                ...
                if actor.id != self.ego.id:
                    if abs(y_local) < self.obs_range / 2 + 1 and x_local < self.obs_range - self.d_behind + 1 and x_local > -self.d_behind - 1:
                        x_pixel, y_pixel, yaw_pixel, l_pixel, w_pixel = get_pixel_info(
                            local_info=(x_local, y_local, yaw_local, l, w), d_behind=self.d_behind, obs_range=self.obs_range, image_size=self.pixor_size
                        )
